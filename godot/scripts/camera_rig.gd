...
func _process(delta: float) -> void:
	...
	var up_vec := Vector3(0.0, 0.0, -1.0) if current_mode == Mode.OVERHEAD else Vector3.UP
	_camera.look_at(_target_look, up_vec)
...
func _apply_overhead_instantly() -> void:
	...
	if _camera:
		...
		_camera.look_at(_target_look, Vector3(0.0, 0.0, -1.0))
