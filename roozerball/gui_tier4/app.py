"""Tier 4 Pygame application — main game loop and event dispatch.

Replaces the Tier 3 application with post-processing effects,
screen shake, enhanced particle emitters, and Tier 4 rendering.

Tier 4 enhancements over Tier 3:
  * Procedural track textures (noise grain, lane markings, scuff marks)
  * Stadium environment (outer wall, railing, floodlight rigs)
  * Crowd silhouettes with subtle animation
  * Post-processing pipeline (bloom, vignette, heat distortion)
  * Screen shake on impacts (camera trauma system)
  * Multi-source floodlight lighting
  * Atmospheric ambient particles
  * Motion blur afterimages on fast figures
  * Metallic ball with specular highlights and afterimage trail
  * Glass-morphism UI panels and dialogs
"""
...
from roozerball.gui_tier4.constants import (
    ...
    MODE_CVC,
    MODE_HVC,
    SHAKE_CANNON_MAGNITUDE,
    SHAKE_CRASH_MAGNITUDE,
    SHAKE_GOAL_MAGNITUDE,
    WINDOW_HEIGHT,
    WINDOW_TITLE,
    WINDOW_WIDTH,
)
from roozerball.gui_tier4.renderer import BoardRenderer, _slot_center, _square_center
from roozerball.gui_tier4.ui import (
    CombatTargetDialog,
    ControlBar,
    EscalateDialog,
    GameOverDialog,
...
class Tier4App:
    """Desktop UI for Roozerball with Tier 4 realistic graphics."""
    ...
    def _trigger_phase_effects(
        ...
        for msg in result.messages:
            ...
            if "cannon" in lower and "fire" in lower:
                ...
                # Tier 4: Screen shake on cannon fire
                self.renderer.trigger_shake(SHAKE_CANNON_MAGNITUDE)
                ...
                for fig in self.game.all_figures():
                    if fig.name in msg:
                        ...
                        if sq is not None:
                            ...
                            # Tier 4: Screen shake on crashes
                            self.renderer.trigger_shake(SHAKE_CRASH_MAGNITUDE)
                        break
        ...
        for name in new_scores:
            if new_scores.get(name, 0) > old_scores.get(name, 0):
                ...
                # Tier 4: Screen shake on goals
                self.renderer.trigger_shake(SHAKE_GOAL_MAGNITUDE)
                break
...
def launch() -> None:
    """Entry point for ``python -m roozerball.gui_tier4``."""
    app = Tier4App()
    app.mainloop()
