"""Tier 4 renderer — realistic board with post-processing and stadium environment.

Enhancements over Tier 3:
  * Procedural track textures (noise grain, lane markings, scuff marks)
  * Multi-source stadium floodlight lighting with soft penumbra shadows
  * Crowd silhouettes in the stands with subtle animation
  * Stadium environment (outer wall, railing, floodlight rigs)
  * Enhanced figure rendering with specular highlights and motion blur
  * Metallic ball with specular reflection and afterimage trail
  * Screen shake on impacts (camera trauma system)
  * Post-processing pipeline (bloom, vignette, heat distortion)
  * Atmospheric ambient particles (dust motes)
"""
...
from roozerball.gui_tier4.effects import PostProcessor
from roozerball.gui_tier4.particles import ParticleSystem
from roozerball.gui_tier4.scene import AnimationController, SceneNode, Transform
from roozerball.gui_tier4.textures import CrowdGenerator, StadiumRenderer, TrackTextureGenerator


# ---------------------------------------------------------------------------
# Camera (enhanced with shake support)
# ---------------------------------------------------------------------------


class Camera:
    """Smooth-scrolling camera with zoom, eased following, and shake offset."""
    ...
    def set_shake(self, ox: float, oy: float) -> None:
    ...
    def world_to_screen(self, wx: float, wy: float) -> Tuple[float, float]:
        sx = (wx - BOARD_CX + self.x) * self.zoom + BOARD_CX + self._shake_offset_x
        sy = (wy - BOARD_CY + self.y) * self.zoom + BOARD_CY + self._shake_offset_y
        return sx, sy
...
class BoardRenderer:
    ...
    Tier 4 adds procedural textures, stadium environment, crowd,
    multi-source lighting, post-processing, and enhanced figure/ball rendering.
    """
    ...
    def __init__(self) -> None:
        ...
        # Tier 4: Procedural textures and environment
        self._track_texture = TrackTextureGenerator()
        self._crowd = CrowdGenerator()
        self._stadium = StadiumRenderer()
        self._track_texture_surface: Optional[pygame.Surface] = None

        # Ball trail positions (for afterimage)
        self._ball_trail: List[Tuple[float, float]] = []
        self._ball_screen_pos: Optional[Tuple[int, int]] = None

        # Previous figure positions for motion blur
        self._prev_figure_screen_pos: Dict[int, List[Tuple[int, int]]] = {}
    ...
    def draw(
        ...
        # Tier 4: Stadium environment (behind everything)
        self._stadium.draw_walls(surface, self._w2s, self.camera.zoom)
        self._draw_track_texture(surface)
        self._draw_procedural_overlay(surface)
        self._draw_squares(surface, game)
        self._draw_ring_gradient_overlay(surface)

        # Tier 4: Crowd silhouettes
        self._crowd.draw(surface, self._w2s, self._time_accum, self.camera.zoom)

        # Tier 4: Multi-source floodlight lighting
        self._draw_floodlight_lighting(surface)
        self._draw_lighting(surface, game)
        self._draw_highlights(surface, game)
        self._draw_shadows(surface, game)

        # Tier 4: Motion blur afterimages (before current figures)
        self._draw_motion_blur(surface, game)

        self._draw_figures(surface, game)
        self._draw_speed_lines(surface, game)

        # Tier 4: Ball afterimage trail
        self._draw_ball_trail(surface, game)
        ...
        # Tier 4: Stadium floodlight rigs (on top layer)
        self._stadium.draw_floodlights(
            surface, self._w2s, self.camera.zoom, self._time_accum,
        )
        ...
        self.post_processor.apply(
            surface,
            ball_screen_pos=self._ball_screen_pos,
            time_ms=self._time_accum,
        )

    # --- Procedural track texture overlay (Tier 4) ---

    def _draw_procedural_overlay(self, surface: pygame.Surface) -> None:
        """Blit the pre-generated track texture with noise grain and markings."""
        if self._track_texture_surface is None:
            self._track_texture_surface = self._track_texture.generate()

        z = self.camera.zoom
        cx, cy = self._w2s(BOARD_CX, BOARD_CY)

        # Scale and position the texture
        tex = self._track_texture_surface
        tw = int(tex.get_width() * z)
        th = int(tex.get_height() * z)
        if tw < 10 or th < 10:
            return
        scaled = pygame.transform.smoothscale(tex, (tw, th))
        # Center on board
        tx = int(cx - BOARD_CX * z)
        ty = int(cy - BOARD_CY * z)
        surface.blit(scaled, (tx, ty), special_flags=pygame.BLEND_ADD)

    # --- Track texture (enhanced with crowd gradient) ---
        ...
        for sector_index, sector in enumerate(game.board.sectors):
            ...
            for ring in Ring:
                ...
                for position in range(sq_count):
                    ...
                    # Tier 4: Per-square lighting from floodlights
                    sq_cx = sum(p[0] for p in raw_pts) / 4
                    sq_cy = sum(p[1] for p in raw_pts) / 4
                    flood_boost = self._floodlight_intensity_at(sq_cx, sq_cy)
                    brightness = min(1.2, brightness + flood_boost * 0.15)
                    ...
                    # Tier 4: Goal glow effect
                    if sq.is_goal:
                        glow_color = GOAL_HOME_GLOW if sq.goal_side.value == "home" else GOAL_VISITOR_GLOW
                        # Pulsing glow
                        pulse = 0.5 + 0.5 * math.sin(self._time_accum * 0.003)
                        glow_alpha = int(20 * pulse)
                        if len(int_pts) >= 3 and glow_alpha > 1:
                            glow_surf = pygame.Surface(
                                (surface.get_width(), surface.get_height()),
                                pygame.SRCALPHA,
                            )
                            pygame.draw.polygon(
                                glow_surf, (*glow_color, glow_alpha), int_pts,
                            )
    ...
    # --- Floodlight lighting (Tier 4) ---

    def _floodlight_intensity_at(self, wx: float, wy: float) -> float:
        """Calculate combined floodlight intensity at a world position."""
        total = 0.0
        for fx, fy in FLOODLIGHT_POSITIONS:
            ...
            dy = wy - ly
            dist_sq = dx * dx + dy * dy
            intensity = FLOODLIGHT_INTENSITY / (1.0 + dist_sq * FLOODLIGHT_FALLOFF)
            total += intensity
        return min(1.0, total)

    def _draw_floodlight_lighting(self, surface: pygame.Surface) -> None:
        """Apply soft floodlight cones from stadium lights."""
        z = self.camera.zoom
        cx, cy = self._w2s(BOARD_CX, BOARD_CY)
        ...
        for fx, fy in FLOODLIGHT_POSITIONS:
            ...
            # Large soft light cone pointing at center
            cone_r = int(280 * z)
            if cone_r < 5:
                continue
            ...
            for layer in range(3):
                r = cone_r - layer * int(cone_r * 0.2)
                if r < 2:
                    continue
                alpha = max(1, int(8 / (layer + 1)))
                pygame.draw.circle(
                    cone_surf,
    ...
    # --- Shadows (enhanced with multi-source soft edges) ---
    ...
    def _draw_shadows(self, surface: pygame.Surface, game: Any) -> None:
        ...
        for fig in game.all_figures():
            ...
            # Multi-layer soft shadow (Tier 4: more layers for softer look)
            for layer in range(4):
                expand = layer * 2
                alpha = max(3, 30 - layer * 8)
                pygame.draw.ellipse(
                    shadow_surf,
                    (0, 0, 0, alpha),
    ...
    def _draw_highlights(self, surface: pygame.Surface, game: Any) -> None:
            ...
            if getattr(fig, "is_towed", False):
                ...
                if biker is not None:
                    ...
                    if bsq is not None:
                        ...
                        # Glowing tow line with energy effect
                        pygame.draw.line(
                            surface, (35, 205, 95),
                        ...
                        pulse = 0.7 + 0.3 * math.sin(self._time_accum * 0.005)
                        bright = int(160 * pulse)
                        pygame.draw.line(
                            surface, (bright, 255, min(255, bright + 40)),
                            (isx, isy), (int(bsx), int(bsy)),
                            max(1, int(1 * z)),
                        )
        ...
        for sector in game.board.sectors:
            for sq in sector.all_squares():
                ...
                sx, sy = self._w2s(wx, wy)
                if sq.is_on_fire:
                    # Tier 4: Embers rising from fire
                    self.particles.emit_embers(sx, sy)
    ...
    def _draw_motion_blur(self, surface: pygame.Surface, game: Any) -> None:
        """Draw fading afterimages of fast-moving figures."""
        z = self.camera.zoom
        for fig in game.all_figures():
            if not fig.has_moved or fig.speed < SPEED_LINE_THRESHOLD:
                continue
            fid = id(fig)
            sq = game.board.find_square_of_figure(fig)
            if sq is None:
            ...
            history = self._prev_figure_screen_pos.setdefault(fid, [])
            history.append((isx, isy))
            if len(history) > MOTION_BLUR_COUNT + 1:
                history.pop(0)
            ...
            if team_side == "home":
                pal = SPRITE_COLORS_HOME.get(
                    ftype, {"fill": (60, 135, 250), "accent": (28, 58, 95), "glow": (95, 165, 255), "specular": (180, 215, 255)},
                )
            else:
                pal = SPRITE_COLORS_VISITOR.get(
                    ftype, {"fill": (240, 70, 70), "accent": (125, 28, 28), "glow": (255, 115, 115), "specular": (255, 195, 195)},
                )

            r = int(14 * z)
            for i, (hx, hy) in enumerate(history[:-1]):
                alpha_frac = (i + 1) / (len(history))
                alpha = int(MOTION_BLUR_ALPHA_STEP * alpha_frac)
                if alpha < 5 or r < 2:
                    continue
                ...
                surface.blit(ghost_surf, (hx - r - 2, hy - r - 2))

    # --- Figures (Tier 4: multi-layer with specular highlights) ---
    ...
    def _draw_figures(self, surface: pygame.Surface, game: Any) -> None:
        ...
        for fig in game.all_figures():
            ...
            if team_side == "home":
                pal = SPRITE_COLORS_HOME.get(
                    ftype,
                    {"fill": (60, 135, 250), "accent": (28, 58, 95), "glow": (95, 165, 255), "specular": (180, 215, 255)},
                )
            else:
                pal = SPRITE_COLORS_VISITOR.get(
                    ftype,
                    {"fill": (240, 70, 70), "accent": (125, 28, 28), "glow": (255, 115, 115), "specular": (255, 195, 195)},
                )
            fill = pal["fill"]
            accent = pal["accent"]
            glow = pal.get("glow", fill)
            specular = pal.get("specular", (255, 255, 255))
            ...
            if fig.needs_stand_up or fig.status in (
                ...
                glow = INJURED_GLOW
                specular = INJURED_SPECULAR

            brightness = RING_BRIGHTNESS.get(sq.ring, 1.0)
            # Tier 4: Add floodlight boost
            flood_boost = self._floodlight_intensity_at(wx, wy)
            brightness = min(1.2, brightness + flood_boost * 0.12)

            fill = _apply_brightness(fill, brightness)
            accent = _apply_brightness(accent, brightness)
            glow = _apply_brightness(glow, brightness)
            specular = _apply_brightness(specular, min(1.3, brightness + 0.2))
            ...
            # Type-specific shape (same as Tier 3)
            if ftype == "biker":
                pts = [
                ...
                pygame.draw.circle(
                    surface, (255, 255, 255),
                    (draw_x, draw_y), draw_r, max(1, int(2 * z)),
                )
                inner_r = int(sr * 0.6)
                if inner_r > 0:
                    pygame.draw.circle(
            ...
            # Tier 4: Specular highlight spot (top-left)
            spec_r = max(1, draw_r // 3)
            spec_x = draw_x - draw_r // 4
            spec_y = draw_y - draw_r // 4
            if spec_r > 0:
                spec_surf = pygame.Surface(
                    (spec_r * 2 + 4, spec_r * 2 + 4), pygame.SRCALPHA,
                )
                pygame.draw.circle(
                    spec_surf, (*specular, 80),
                    (spec_r + 2, spec_r + 2), spec_r,
                )
                surface.blit(
                    spec_surf,
                    (spec_x - spec_r - 2, spec_y - spec_r - 2),
                    special_flags=pygame.BLEND_ADD,
                )
            ...
            if fig.has_ball:
                ...
                if br > 0:
                    ...
                    pygame.draw.circle(
                        surface, (255, 125, 28), (bx + br, by + br), br,
                    )
                    # Specular on ball indicator
                    pygame.draw.circle(
                        surface, (255, 250, 240),
                        (bx + br, by + br), br, max(1, int(z)),
                    )
                    tiny_spec = max(1, br // 3)
                    pygame.draw.circle(
                        surface, (255, 255, 255),
                        (bx + br - br // 3, by + br - br // 3), tiny_spec,
                    )
    ...
    # --- Speed lines (Tier 4: enhanced with fade gradient) ---
    ...
    def _draw_speed_lines(self, surface: pygame.Surface, game: Any) -> None:
        ...
        for fig in game.all_figures():
            ...
            trail_angle = sector_angle + math.pi
            ...
            for i in range(SPEED_LINE_COUNT):
                offset_angle = trail_angle + (i - 1.5) * 0.12
                ex = center + math.cos(offset_angle) * line_len
                ey = center + math.sin(offset_angle) * line_len
                alpha = max(15, SPEED_LINE_ALPHA - i * 22)
                # Tier 4: Team-tinted speed lines
                team_side = fig.team.value
                if team_side == "home":
                    color = (160, 200, 255, alpha)
                else:
                    color = (255, 180, 160, alpha)
                pygame.draw.line(
                    line_surf, color,
                    (center, center), (int(ex), int(ey)),
                    max(1, int(z * 1.5)),
                )
            surface.blit(
                line_surf,
    ...
    # --- Ball trail (Tier 4: afterimage) ---

    def _draw_ball_trail(self, surface: pygame.Surface, game: Any) -> None:
        """Draw fading afterimage trail behind the ball."""
        ...
        if game.ball.carrier is not None:
            ...
            sq = game.board.get_square(
                game.ball.sector_index, game.ball.ring, game.ball.position,
            )
            wx, wy = _square_center(sq)

        sx, sy = self._w2s(wx, wy)
        self._ball_trail.append((sx, sy))
        if len(self._ball_trail) > BALL_TRAIL_LENGTH:
        ...
        for i, (tx, ty) in enumerate(self._ball_trail[:-1]):
            frac = (i + 1) / len(self._ball_trail)
            alpha = int(BALL_TRAIL_ALPHA_STEP * frac)
            trail_r = max(1, int(r * frac * 0.7))
            if alpha < 3 or trail_r < 1:
                continue
            trail_surf = pygame.Surface(
                (trail_r * 2 + 4, trail_r * 2 + 4), pygame.SRCALPHA,
            )
            pygame.draw.circle(
                trail_surf, (*ball_color, alpha),
                (trail_r + 2, trail_r + 2), trail_r,
            )
            surface.blit(trail_surf, (int(tx) - trail_r - 2, int(ty) - trail_r - 2))

    # --- Ball (Tier 4: metallic with specular and shimmer) ---

    def _draw_ball(self, surface: pygame.Surface, game: Any) -> None:
        z = self.camera.zoom
        if game.ball.state.value == "not_in_play":
            self._ball_screen_pos = None
            return
        ...
        # Ball body — gradient fill (darker edge, lighter center)
        pygame.draw.circle(surface, ball_color, (isx, isy), r)

        # Tier 4: Inner gradient for 3D metallic look
        inner_r = max(1, int(r * 0.6))
        lighter = _lerp_color(ball_color, (255, 255, 255), 0.25)
        pygame.draw.circle(surface, lighter, (isx - r // 5, isy - r // 5), inner_r)
        ...
        # Tier 4: Specular highlight (bright white spot)
        spec_r = max(1, r // 3)
        pygame.draw.circle(
            surface, BALL_SPECULAR,
            (isx - r // 3, isy - r // 3), spec_r,
        )
        # Secondary smaller specular
        spec2_r = max(1, r // 5)
        pygame.draw.circle(
            surface, (255, 255, 255),
            (isx + r // 4, isy - r // 5), spec2_r,
        )
    ...
    def _draw_canvas_scoreboard(
        ...
        # Tier 4: Glass-morphism scoreboard box
        ...
        box_surf.fill((16, 22, 36, 210))
        surface.blit(box_surf, box_rect.topleft)
        # Subtle top border glow
        pygame.draw.line(
            surface, (80, 110, 160),
            (box_rect.left, box_rect.top),
            (box_rect.right, box_rect.top), 1,
        )
        pygame.draw.rect(surface, (55, 70, 95), box_rect, 1)
    ...
    def emit_crash_particles(self, wx: float, wy: float) -> None:
        ...
        # Tier 4: Also emit sparks
        self.particles.emit_sparks(sx, sy)
    ...
    def trigger_shake(self, magnitude: float) -> None:
        """Trigger screen shake from camera trauma."""
        self.post_processor.screen_shake.add_trauma(magnitude)
